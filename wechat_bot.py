...
def text_reply(msg):
    itchat.send('这是我的小号，暂无调戏功能，有事请加我大号：Bang', msg['FromUserName'])
    ...
    if msg['Type'] == TEXT:
        for item in chatrooms:
            if not item['UserName'] == chatroom_id:
                itchat.send('%s\n%s' % (username, msg['Content']), toUserName=_sync_name)
    elif msg['Type'] == SHARING:
        for item in chatrooms:
            if not item['UserName'] == chatroom_id:
                itchat.send('%s\n%s\n%s' % (username, msg['Text'], msg['Url']), toUserName=_sync_name)
